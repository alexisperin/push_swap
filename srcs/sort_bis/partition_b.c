...
/*   Updated: 2022/11/18 09:23:42 by aperin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	partition_end(t_stack **stack_a, t_stack **stack_b, int part)
{
	t_stack	*tmp;
...
static void	threeway_partition(t_stack **stack_a, t_stack **stack_b, int size)
{
	...
	partition_end(stack_a, stack_b, partition);
}
...
static void	twoway_partition(t_stack **stack_a, t_stack **stack_b, int size)
{
	...
	while (to_push > 0)
	{
		...
		{
			(*stack_b)->partition = (partition * 3) + 2;
			rb(stack_a);
		}
	}
	partition_end(stack_a, stack_b, partition);
...
void	partition_b(t_stack **stack_a, t_stack **stack_b)
{
	...
	if (size <= 3)
	{
		while ((*stack_b)->partition == partition)
			pa(stack_a, stack_b);
	}
	else if (size <= 6)
